  ...
  createRouter(): Router {
    ...
    router.get("/students/competency-progress", requireAuth, async (req: AuthenticatedRequest, res) => {
      try {
        ...
        // Get all grades for this student with competency information
        const studentGrades = await db
          .select({
            ...
            competencyId: sql`competencies.id`.as('competencyId'),
            competencyName: sql`competencies.title`.as('competencyName'),
            componentSkillName: sql`component_skills.title`.as('componentSkillName'),
          })
          .from(gradesTable)
          .innerJoin(sql`component_skills`, eq(gradesTable.componentSkillId, sql`component_skills.id`))
          .innerJoin(sql`competencies`, eq(sql`component_skills.competency_id`, sql`competencies.id`))
          .where(and(
            eq(gradesTable.studentId, studentId),
            ne(gradesTable.componentSkillId, sql`NULL`)
          ))
          .orderBy(desc(gradesTable.gradedAt));

        // Group grades by competency and component skill
        ...
        studentGrades.forEach((grade) => {
          if (grade.componentSkillId && grade.score) {
            const key = `${grade.competencyId}-${grade.componentSkillId}`;
            const score = parseFloat(grade.score);
            
            if (!progressMap.has(key)) {
              progressMap.set(key, {
                competencyId: Number(grade.competencyId),
                competencyName: String(grade.competencyName),
                componentSkillId: grade.componentSkillId,
                componentSkillName: String(grade.componentSkillName),
                scores: [],
                gradedDates: []
              });
            }
            ...
            entry.gradedDates.push(new Date(grade.gradedAt));
          }
        });

        // Calculate progress for each component skill
        const competencyProgress = Array.from(progressMap.values()).map(entry => {
          const scores = entry.scores.sort((a, b) => b - a); // Sort descending
          ...
          if (scores.length >= 2) {
            const recentAverage = scores.slice(0, Math.ceil(scores.length / 2)).reduce((sum, score) => sum + score, 0) / Math.ceil(scores.length / 2);
            const olderAverage = scores.slice(Math.floor(scores.length / 2)).reduce((sum, score) => sum + score, 0) / Math.floor(scores.length / 2);
          ...
          return {
            ...
            componentSkillId: entry.componentSkillId,
            componentSkillName: entry.componentSkillName,
            averageScore: Math.round(averageScore * 10) / 10, // Round to 1 decimal
            totalScores,
            lastScore,
            lastUpdated: entry.gradedDates[0]?.toISOString() || new Date().toISOString(),
            progressDirection
          };
        });
